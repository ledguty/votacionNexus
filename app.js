...
// Asegúrate de tener tu API_KEY de Gemini
const API_KEY = 'AIzaSyD4FofSiMruEGOAnE50GMAA6djyfKx9gwU';
const API_URL = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent';

// ... (código existente)

const formalizeProposalBtn = document.getElementById('formalizeProposalBtn');
const formalProposalResult = document.getElementById('formalProposalResult');
const formalProposalText = document.getElementById('formalProposalText');
...
if (approvedProposalMessage) {
    ...
    if (approvedProposal) {
        approvedProposalMessage.innerHTML = `
            <p><strong>Usted aprobó la propuesta:</strong> "${approvedProposal}"</p>
            <p>Cree la votación para que los usuarios voten o formalice la propuesta con IA.</p>
        `;
        approvedProposalMessage.classList.remove('hidden');
        formalizeProposalBtn.classList.remove('hidden');
    }
}

if (formalizeProposalBtn) {
    formalizeProposalBtn.addEventListener('click', async () => {
        const approvedProposal = sessionStorage.getItem('approvedProposal');
        if (approvedProposal) {
            try {
                const formalProposal = await getFormalProposal(approvedProposal);
                formalProposalText.textContent = formalProposal;
                formalProposalResult.classList.remove('hidden');
                sessionStorage.setItem('formalProposal', formalProposal);
            } catch (error) {
                console.error('Error al formalizar la propuesta:', error);
                alert('Hubo un error al formalizar la propuesta. Por favor, inténtelo de nuevo.');
            }
        }
    });
}

async function getFormalProposal(proposal) {
    const prompt = `Formaliza la siguiente propuesta de votación, haciéndola más profesional y detallada: "${proposal}"`;
...
// Modifica la función de crear nueva votación para usar la propuesta formalizada si está disponible
if (createVotingForm) {
    createVotingForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const title = document.getElementById('votingTitle').value;
        const description = document.getElementById('votingDescription').value || 
                            sessionStorage.getItem('formalProposal') || 
                            sessionStorage.getItem('approvedProposal') || '';

        // ... (resto del código para crear la votación)

        // Después de crear la votación exitosamente:
        sessionStorage.removeItem('approvedProposal');
        sessionStorage.removeItem('formalProposal');
        formalProposalResult.classList.add('hidden');
    });
}
